...
class CcaixaPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `caixa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM caixa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO caixa (date, title, link, content)"
